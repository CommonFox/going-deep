...
| `game_environment.ipynb` | Whether implied totals and gamescript predict beyond a player's own recent scoring: gamescript/`implied_margin` does for RB and TE (confirming half the RB-favorite/WR-underdog folk model, not all of it), wind does for QB and WR (the deep-passing half of the wind claim), implied team total doesn't survive a baseline swap, and beyond-the-projection stays untestable (#117's 2026 gap). |
