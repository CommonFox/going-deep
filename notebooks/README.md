...
| `first_pick.ipynb` | Who to take at 1.01 in Sleeper: whether the pick matters next to the opening plan, what the board says about Bijan, Gibbs and Allen in isolation, why the quarterback loses to the pick you'd make at 29 instead, the paired simulation and the plan grid it survives, why that simulation cannot separate two backs at all, and the touchdown regression that makes the two backs level. |
...
| `defense_matchup.ipynb` | Whether "defense vs. position" predicts beyond a player's own recent scoring: yes, per position (QB > RB > TE > WR, against the common TE/DST claim), not at all pooled, not yet testable against the vendor projection (#117's 2026 gap), and no games-observed threshold needed — a disjoint-bucket read that looks like a ramp-up turns out to be bucket noise. |
| `game_environment.ipynb` | Whether implied totals, gamescript and conditions predict beyond a player's own recent scoring: gamescript/`implied_margin` does for RB and TE (confirming half the RB-favorite/WR-underdog folk model, not all of it), wind and roof shelter do for QB/WR and WR respectively (the deep-passing half of the wind claim), implied team total and raw temperature don't survive a baseline swap, and beyond-the-projection stays untestable (#117's 2026 gap). |
| `role_trend.ipynb` | Whether a player's role trend (`player_role_trend`) predicts beyond his own recent scoring: level doesn't — it's measured concurrently with the outcome it's compared to, confirmed by testing it against the player's own next game instead. Direction flips sign depending on which recent-form baseline holds it fixed, at every window from 1-6 games, and the flip gets larger rather than smaller at a rest-of-season horizon — no actionable "N weeks of decline" rule survives. |
| `verdicts.ipynb` | One promotion rule, stated before looking, applied identically to all three weekly signals: `|t| >= 2.0` against the vendor's own weekly projection, on at least a season's worth of clustered weeks. Nothing clears it today — not a rejection, a structural non-event, since the 2026 archive holds exactly one played week, too few for the harness's own significance test to even run. Reconciles against each signal's own verdict (nothing flips) and states what unblocks the real answer. |
